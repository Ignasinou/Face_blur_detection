...
class BlurScore:
    ...
    def _preprocess(self, img, do_gray = True):
        """
        图像的读取及预处理操作
        :param imgName: 图像的名称
    ...
    def _Brenner(self, face):
        """
        指标一：Brenner梯度函数
        :param imgName: 图像的名称
        :return: 模糊度分数
        """
        img2gray, cropImg = self._preprocess(face, do_gray = True)
    ...
    def _Laplacian(self, face):
        """
        指标二：拉普拉斯方差算法
        :param imgName: 图像的名称
        :return: 模糊度分数
        """
        cropImg = self._preprocess(face, do_gray = False)
        score = cv2.Laplacian(cropImg, cv2.CV_64F).var()
        return score


    def _Thenengrad(self, face):
        """
        指标三：Thenengrad梯度函数
        :param imgName: 图像的名称
        :return: 模糊度分数
    ...
    def _Gaussian_Laplacian(self, face):
        '''
        指标十二：
        对采集到的人脸图像进行如下处理：
        1.高斯模糊去噪，
        ...
        # 高斯滤波
        gauss_blur = cv2.GaussianBlur(face, (3, 3), 0)
